  ...
  const fetchLetters = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/letters?search_query=${keyword}&page=${page}&limit=${limit}`
      );
                  ...
                  <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                    {letters.map((item, index) => (
                      <tr key={item.No}>
                        ...
                        {/* <td className="px-4 py-4 text-sm whitespace-nowrap">
                          <Link
                            to={`/letter/detail/${item?.uuid}`}
                            className="p-2 text-gray-500 transition-colors duration-200 rounded-lg dark:text-gray-300 hover:bg-gray-100"
                          >
                            Detail
                          </Link>
                        </td> */}
