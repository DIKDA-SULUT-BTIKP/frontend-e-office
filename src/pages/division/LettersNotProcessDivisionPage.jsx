...
const LettersNotProcessPage = () => {
  ...
  const [division, setDivision] = useState("BTIKP");
  const fetchLetters = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/letters/not-process/division?search_query=${keyword}&page=${page}&limit=${limit}&division=${division}`
      );
                  ...
                  <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                    {letters.map((item, index) => (
                      <tr key={item.No}>
                        ...
                        <td className="px-4 py-4 text-sm whitespace-nowrap">
                          <Link
                            to={`/division/letter/process/${item.uuid}`}
...
export default LettersNotProcessPage;
